...
/** Historical name — the shared error class both providers throw (pipeline branches on .kind). */
export { LlmError as OllamaError }
export type { LlmErrorKind as OllamaErrorKind } from './http'
...
export class OllamaClient {
  ...
  async listModels(): Promise<string[]> {
    ...
    if (!base) {
      this.setStatus({ reachable: false })
      throw new LlmError('network', 'Ollama base URL must be loopback (localhost / 127.0.0.1 / [::1])')
    }
    let res: { ok: boolean; status: number; text: string }
    try {
      res = await this.request(`${base}/api/tags`, { method: 'GET' }, TAGS_TIMEOUT_MS)
    } catch (err) {
      if (err instanceof LlmError && (err.kind === 'network' || err.kind === 'timeout')) {
        this.setStatus({ reachable: false })
      }
      throw err
    }
    if (!res.ok) {
      ...
      throw new LlmError('http', `GET /api/tags returned ${res.status}`, res.status)
    }
    let names: string[]
    try {
      ...
      this.setStatus({ reachable: true })
      throw new LlmError('http', 'malformed /api/tags response', res.status)
    }
  ...
  async chat(opts: ChatOptions): Promise<string> {
    ...
    this.setStatus({ reachable: true })
    if (!res.ok) throw new LlmError('http', `POST /api/chat returned ${res.status}`, res.status)
    let content: unknown
    try {
      const json: unknown = JSON.parse(res.text)
      content = isRecord(json) && isRecord(json.message) ? json.message.content : undefined
    } catch {
      throw new LlmError('http', 'malformed /api/chat response', res.status)
    }
  ...
  /** Shared transport (llm/http.ts) with the Ollama response-size cap applied. */
  private request(
    url: string,
    ...
    return llmRequest(url, init, timeoutMs, OLLAMA_RESPONSE_MAX_BYTES, external)
  }
